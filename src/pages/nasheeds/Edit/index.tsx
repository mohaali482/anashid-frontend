...
import { addNasheed, fetchNasheed, resetErrors, updateNasheed } from "../../../redux/ducks/nasheedSlice";
...
const NasheedForm = () => {
    ...
    useEffect(() => {
        if (id !== undefined) {
            dispatch(fetchNasheed(parseInt(id)))
        }
    }, [])

    useEffect(() => {
        if (nasheed !== null && audioRef.current !== null) {
            audioRef.current.src = nasheed.audio
    ...
    const onSubmit = (e: React.FormEvent) => {
        ...
        setSubmit(true)
        dispatch(updateNasheed({ id: parseInt(id), formData }))
    }
            ...
            <StyledForm onSubmit={onSubmit} ref={formRef}>
                <StyledInputDiv>
                    <StyledFileUpload style={{
                        ...
                        {poster || nasheed?.poster ?
                            <Image src={posterPreview || nasheed?.poster} /> :
                            (
                                <>
                ...
                <StyledInputDiv>
                    <StyledInput required placeholder="Nasheed Title" type="text" name="name" id="name" defaultValue={nasheed?.name} />
                    {formErrors.name &&
                ...
                <StyledInputDiv>
                    <StyledTextArea required placeholder="Nasheed Description" name="description" id="description" rows={10} defaultValue={nasheed?.description}>
                    </StyledTextArea>
                </StyledInputDiv>
                <StyledInputDiv>
